...
enum ENUMOP { OP_RECV, OP_SEND, OP_ACCEPT, OP_RANDOM_MOVE, OP_PLAYER_MOVE, OP_NPC_MOVE_END };
...
void activate_npc(int id)
{
	...
	atomic_compare_exchange_strong(&g_clients[id].m_status, &old_state, ST_ACTIVE);
...
void worker_thread()
{
	while (true) {
		...
		case OP_ACCEPT: {
			...
			AcceptEx(l_socket, c_socket, exover->io_buf, NULL,
				sizeof(sockaddr_in) + 16, sizeof(sockaddr_in) + 16, NULL, &exover->over);
					  ...
					  break;
		case OP_RANDOM_MOVE: {
			if (exover->rest_move == 0) {
				EXOVER* over = new EXOVER;
				over->op = OP_NPC_MOVE_END;
				over->p_id = exover->p_id;
				...
				if (true == keep_alive) add_timer(user_id, OP_RANDOM_MOVE, 1000, exover->p_id, exover->rest_move - 1);
		...
		case OP_NPC_MOVE_END: {
			...
			g_clients[user_id].is_moving = false;
			delete exover;
		}
							break;
		default:
...
int API_NPC_Move_Start(lua_State* L) {
	int my_id = (int)lua_tointeger(L, -2);
	int user_id = (int)lua_tointeger(L, -1);
	if (!g_clients[my_id].is_moving) {
		add_timer(my_id, OP_RANDOM_MOVE, 1000, user_id, MOVE_COUNT);
		g_clients[my_id].is_moving = true;
	}
	lua_pop(L, 2);
	return 0;
}
...
void init_npc()
{
	for (int i = NPC_ID_START; i < NPC_ID_START + NUM_NPC; ++i) {
		...
		lua_register(L, "API_npc_move_start", API_NPC_Move_Start);
	...
	while (true) {
		...
		while (true) {
			...
			case OP_RANDOM_MOVE:
				EXOVER* over = new EXOVER;
				over->op = ev.event_id;
				over->rest_move = ev.rest_moveCount;
